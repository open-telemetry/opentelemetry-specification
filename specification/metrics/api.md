...
A _Label_ is the term used to refer to a key-value attribute associated with a metric event, similar to a [Span attribute](../trace/api.md#span) in the tracing API.

Each of the instrument calling conventions detailed in the [user-level API specification](api-user.md) accept a set of labels as an argument.

### Meter Interface

To produce measurements using an instrument, you need an SDK that implements the `Meter` API.
This interface consists of a set of instrument constructors, and a facilities for capturing batches of measurements in a semantically atomic way.
...
A single Observer instrument callback can capture multiple metric events associated with different sets of labels.
...
Observer instruments capture not only current values, but also effectively _which labels are current_ at the moment of collection.
...
When handling a request, compute a LabelSet containing the name of the protocol and potentially other useful labels, then call `Add()` with the same labels and the number of bytes read.

To lower the cost of this reporting, you can `Bind()` the instrument with each of the supported protocols ahead of time.
...
- Why not report deltas in the Observer callback.
  ...
  Nothing prevents reporting deltas with an Observer, but the standard aggregation for Observer instruments is to sum the current value across distinct labels.
...
These can be aggregated across hosts to compute cluster-wide memory holdings by shard, for example, using the standard aggregation for Observers, which sums the current value across distinct labels.
