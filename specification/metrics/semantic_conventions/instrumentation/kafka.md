...
|                                      |                    |            |                  |                     |                                                                      | `topic`     | topic name        |
| kafka.producer.record-error-rate     | Asynchronous Gauge | Double     | error rate | `{errorrate}` | The average per-second number of record sends that resulted in errors for a specific topic.  | `client-id` | `client-id` value |
|                                      |                    |            |            |               |                                                                                                | `topic`     | topic name        |
