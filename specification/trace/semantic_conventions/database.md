...
Since SQL statements may have very high cardinality even without arguments, SQL spans MAY be named the following way:
`<db.operation> <db.name>.<db.sql.table>`, provided that `db.operation` and/or `db.sql.table` are available.
