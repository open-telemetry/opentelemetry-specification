...
In languages that can handle a large number of optional parameters well
(e.g. by supporting named and default arguments or if the language already
uses the builder pattern),
`RecordException` MAY accept an additional boolean parameter to set the
`exception.escaped` attribute on the event.
