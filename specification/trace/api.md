...
This is a specialized variant of [`AddEvent`](#add-events), so for anything not specified here
the same requirements as for `AddEvent` apply.

The signature of the method is to be
...
If provided, `RecordException` MUST provide an optional parameter to provide
any additional event attributes
...
Note: For example, if `exception.stacktrace=""` is provided,
the stacktrace will be an empty string, which allow `RecordException` to skip
collecting it, speeding up the exception recording.

Note: `RecordException` may be seen as a renamed overload of `AddEvent` with
with additional exception-specific parameters and all other parameters being
optional (because they have defaults from the exception semantic convention).
