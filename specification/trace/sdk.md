...
* `onQueueFull` - the behavior when a new span is received and the queue is full.
  The possible values are `DROP` and `BLOCK`. If set to `DROP`, spans are
  dropped. If set to `BLOCK`, adding the span waits until space is available in
  the queue or until `blockOnQueueFullTimeoutMillis` elapses. The default value
  is `DROP`.
