...
- OpenTelemetry defines an SPI (Service Provider Interface)
  for [emitting LogRecords](./spi.md#emit-logrecord). Application developers are
  NOT encouraged to call this SPI directly. It is provided for library authors
  to build [Appenders](./spi.md#how-to-create-log4j-style-appender), which use
  the SPI to bridge between existing logging libraries and the OpenTelemetry log
...
- OpenTelemetry defines an API for [emitting Events](./event-api.md). The API
  consists of convenience methods which delegate to the Log SPI
  for [emitting LogRecords](./spi.md#emit-logrecord). Application developers are
  encouraged to call this Events API directly.

- OpenTelemetry defines an [SDK](./sdk.md) implementation of the [SPI](./spi.md),
...
an [SPI](./spi.md) and [SDK](./sdk.md), which can be used together with existing
...
modifying each logging statement, [Appenders](./spi.md#how-to-create-log4j-style-appender)
use the SPI to bridge logs from existing logging libraries to the OpenTelemetry
...
![Application, SPI, SDK Diagram](img/application-spi-sdk.png)
