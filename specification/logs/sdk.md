...
* **Status**: [Development](../document-status.md) - `onQueueFull` - the behavior
  when a new log record is received and the queue is full. The possible values
  are `DROP` and `BLOCK`. If set to `DROP`, log records are dropped. If set to
  `BLOCK`, adding the log record waits until space is available in the queue or
  until `blockOnQueueFullTimeoutMillis` elapses. The default value is `DROP`.
* **Status**: [Development](../document-status.md) -
