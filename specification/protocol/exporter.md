...
| Endpoint (OTLP/gRPC) | Target to which the exporter is going to send spans or metrics. The endpoint SHOULD accept any form allowed by the underlying gRPC client implementation. Additionally, the endpoint MUST accept a URL with a scheme of either `http` or `https`. A scheme of `https` indicates a secure connection. If the endpoint does not include a scheme then the `insecure` configuration option indicates whether a secure connection should be used. If the gRPC client implementation does not support an endpoint with a scheme of `http` or `https` then the endpoint SHOULD be transformed to the most sensible format for that implementation. | `https://localhost:4317` | `OTEL_EXPORTER_OTLP_ENDPOINT` `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` |
| Insecure             | Whether to enable client transport security for the exporter's gRPC connection when the `endpoint` does not include a scheme. If a scheme is included with the `endpoint` then the scheme takes precedence over the `insecure` configuration option. The `insecure` option only applies to OTLP/gRPC - OTLP/HTTP always uses the scheme provided for the `endpoint`. The `insecure` configuration MUST be implemented if `endpoint` configuration without a scheme is allowed. | `false` | `OTEL_EXPORTER_OTLP_INSECURE` `OTEL_EXPORTER_OTLP_SPAN_INSECURE` `OTEL_EXPORTER_OTLP_METRIC_INSECURE` |
