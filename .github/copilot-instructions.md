...
- If a change adds, removes, or tightens normative requirements, verify that the
  affected document or section is marked as `Development`, `Alpha`, or `Beta`
  (per [document status](../specification/document-status.md)); otherwise, flag
  it unless the PR explains an approved stability exception.
