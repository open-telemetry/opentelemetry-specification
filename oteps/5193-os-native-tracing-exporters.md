...
The near-zero disabled cost is fully realized for logs today; the equivalent gates for spans and metrics do not exist yet and are discussed later in this document.
...
1. When nothing is listening, emission is nearly free. The processor consults the facility's enablement state; if no session has subscribed, `Enabled()` is false and the instrumentation does almost no work (see ["The disabled fast path for Logs"](#the-disabled-fast-path-for-logs)).
...
A conventional exporter behind a built-in processor does not fit these two signals. The *batch* processor buffers, which is the opposite of the immediate per-emit write we want. The *simple* processor must serialize export calls — the exporter contract forbids concurrent `Export()` invocations — discarding the concurrent per-CPU emission these facilities are built for. A new concurrent per-emit processor wrapping a synchronous exporter was considered and is not proposed: the processor/exporter split earns its place when there is work between emission and export — buffering, batching, retry — and here there is none, so the split adds an indirection with no semantic content, while the exporter interface's non-concurrent contract would need changing regardless. The write is therefore modeled as the processor itself: the emit is the processing.
