...
The disabled path is cheap because the OS-native log processor implements the `LogRecordProcessor` `Enabled` operation, answering it from the facility's live enablement state; when instrumentation checks `Logger.Enabled` before emitting (the pattern this component is designed for), a disabled log is never constructed. OpenTelemetry Rust contrib benchmarks ([`user_events`](https://github.com/open-telemetry/opentelemetry-rust-contrib/blob/main/opentelemetry-user-events-logs/benches/logs.rs), [ETW](https://github.com/open-telemetry/opentelemetry-rust-contrib/blob/main/opentelemetry-etw-logs/benches/logs.rs)) measure roughly 8 ns for a disabled log on this path, measured through the `tracing` macro with the `opentelemetry-appender-tracing` bridge; absolute numbers vary by language and implementation. This applies to logs today; for spans, `Tracer.Enabled` exists but `SpanProcessor` has no `Enabled` hook for this component to answer it, and metrics have no equivalent gate (see ["What the model provides"](#what-the-model-provides)). Note that `Logger.Enabled` returns false only when all registered processors implement `Enabled` and each returns false: a coexisting processor that would accept the record, or one that does not implement `Enabled` at all, causes the record to be constructed regardless of the OS-native processor's state. The full disabled fast path is therefore realized when the OS-native processor is the only processor for the routed telemetry; defining how pipelines are composed or routed so that mixed deployments retain it is part of the specification phase.
...
- When collection is enabled, the per-event cost is paid on the emitting thread. Each event is serialized and written synchronously on the application thread that emitted it, whereas the batching OTLP path moves serialization and export off the hot path onto a background thread. The cost is bounded, with no I/O wait, but it is not zero: contrib benchmarks (values from the comments in the benchmark sources; hardware-dependent) measure roughly 0.5-0.6 µs per enabled log record for Rust [`user_events`](https://github.com/open-telemetry/opentelemetry-rust-contrib/blob/main/opentelemetry-user-events-logs/benches/logs.rs), roughly 1.4-1.6 µs for Rust [ETW](https://github.com/open-telemetry/opentelemetry-rust-contrib/blob/main/opentelemetry-etw-logs/benches/logs.rs), and roughly 0.9 µs for a .NET [ETW export](https://github.com/open-telemetry/opentelemetry-dotnet-contrib/blob/main/test/OpenTelemetry.Exporter.Geneva.Benchmarks/Exporter/TLDLogExporterBenchmarks.cs) (TraceLogging encoding; ~1.1 µs end to end from the `ILogger` call). This cost is paid only while a consumer has collection enabled; when nothing is listening it drops to nanoseconds (see ["The disabled fast path for Logs"](#the-disabled-fast-path-for-logs)). Workloads emitting at extreme sustained rates should weigh this per-event, caller-thread cost against the amortized cost of the batching OTLP path; for most, it is far below the cost the same record incurs downstream.
...
- Does the synchronous write block the application's hot path?
  ...
  There is no I/O wait: emitting is a write to an in-memory, kernel-backed buffer on the calling thread, with no disk I/O and no network call, so the calling thread never stalls on a device or a peer. The bounded per-event work of serializing and writing the event does run on the calling thread: contrib benchmarks measure it at roughly 0.5-1.6 µs per log record while collection is enabled, depending on facility and language (see [Trade-offs and mitigations](#trade-offs-and-mitigations)). When nothing is listening the producer returns without building the record, at a cost of a few nanoseconds.
