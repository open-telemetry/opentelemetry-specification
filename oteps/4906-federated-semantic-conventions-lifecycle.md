...
A federated registry like `jvm-metrics` would contain a manifest, the convention definitions, and a policy enforcement GitHub Action.
...
- The registry MUST use a dependabot or renovate bot to keep dependencies up-to-date.
...
3. **Criteria for Promotion**: To be merged into core `semconv`, a federated registry MUST:
    - Reach `stable` status, and remain backward compatible for a full year.
...
Promotion to core provides the highest level of visibility, stability guarantees, and long-term maintenance by the Semantic Conventions SIG. It also allows other federated semantic convention
...
tooling (`weaver`) to ensure that stability. We believe that once past initial setup, the overall maintenance burden for instrumentation authors will be less.

### 6. Where is the documentation for all these registries?

The "Platform Release" manifest serves as the primary technical discovery mechanism. We also intend to expand the OpenTelemetry site to include a central "Registry of Registries" where users can browse all approved federated conventions, their stability status, and documentation.
...
expect instrumentation libraries to publish the schema URL they use, and
...
A central "Registry of Registries" to discover available conventions, and to advertise conventions from non-OpenTelemetry sources, similar to the instrumentation registry on opentelemetry.io.
